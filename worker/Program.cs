    ...
    public class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                string postgresDatabase = Environment.GetEnvironmentVariable("POSTGRES_DATABASE") ?? "postgres";
                string postgresPort = Environment.GetEnvironmentVariable("POSTGRES_PORT") ?? "5432"; // Default to 5432
                string redisHost = Environment.GetEnvironmentVariable("REDIS_HOST") ?? "redis";

                // Build the PostgreSQL connection string with the provided environment variables
                string postgresConnectionString = $"Host={postgresHost};Username={postgresUser};Password={postgresPassword};Database={postgresDatabase};Port={postgresPort}";
